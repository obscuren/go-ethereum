...
func (evm *EVM) Run(contract *Contract, input []byte) (ret []byte, err error) {
	...
	defer func() {
		if err != nil {
			evm.log(pc, op, contract.Gas, cost, mem, stack, contract, err)
		}
	}()
	...
	for ; ; instrCount++ {
		...
		*/
		...
		// Add a log message
		evm.log(pc, op, contract.Gas, cost, mem, stack, contract, nil)
...
func (evm *EVM) log(pc uint64, op OpCode, gas, cost *big.Int, memory *Memory, stack *stack, contract *Contract, err error) {
	if evm.cfg.Debug {
		var mem []byte
		if !evm.cfg.Logger.DisableMemory {
			mem = make([]byte, len(memory.Data()))
			copy(mem, memory.Data())
		}

		var stck []*big.Int
		if !evm.cfg.Logger.DisableStack {
		...
		if !evm.cfg.Logger.DisableStorage {
			contract.self.ForEachStorage(evm.cfg.Logger.FullStorage, func(key, value common.Hash) bool {
				storage[key] = value
				return true
			})
		}
