...
// AddStructuredLog adds a structered log.
func (t *TraceCollector) AddStructuredLog(slog vm.StructLog) {
...
// TraceTransaction returns the structured logs created duritg the execution of EVM
// and returns them as a JSON object.
func (s *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger VmLoggerOptions) (*ExecutionResult, error) {
	structLogs, ret, gas, err := s.traceTransaction(txHash, logger)
	if err != nil {
		return nil, err
	}
...
func (s *PrivateDebugAPI) traceTransaction(txHash common.Hash, logger VmLoggerOptions) ([]vm.StructLog, []byte, *big.Int, error) {
